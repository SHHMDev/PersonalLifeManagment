...
export const tasksRepository = {
  ...
  async create(payload: Omit<Task, 'id' | 'isDone' | 'completedAt'>): Promise<void> {
    await sqliteService.run(
      ...
      [payload.categoryId, payload.title, payload.description, payload.importance, payload.urgency, payload.benefit, payload.priorityScore, payload.createdAt]
    );
  },
  async update(task: Task): Promise<void> {
    await sqliteService.run(
      ...
      [task.categoryId, task.title, task.description, task.importance, task.urgency, task.benefit, task.priorityScore, task.isDone, task.completedAt, task.id]
    );
  },
